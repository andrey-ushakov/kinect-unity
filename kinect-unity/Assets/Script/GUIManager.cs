...
public class GUIManager : MonoBehaviour {
	...
	void initHearts(){
		...
		for (int i=0; i<fullHeartNum; i++) {
			...
			go_newHeart.GetComponent<GUITexture>().pixelInset = new Rect(heartMarginX+heartSpacingX*i, heartMarginY, heartSize, heartSize); 
	...
	void updateHearts(){
		...
		for(int i=0;i<fullHeartNum;i++){
			hearts[i].GetComponent<GUITexture>().texture = fullHeartTex;
		}
		if (halfHeartNum == 1) {
			hearts[fullHeartNum].GetComponent<GUITexture>().texture = halfHeartTex;
		}
		if (fullHeartNum + halfHeartNum < totalHeartNum) {
			for(int i=fullHeartNum + halfHeartNum; i<totalHeartNum;i++){
				hearts[i].GetComponent<GUITexture>().texture = emptyHeartTex;
			}
		}
	}
